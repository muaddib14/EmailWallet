...
export function NavConnectButton() {
  const { address, step, error, connectAndSign, signOut } = useWalletAuth();
  ...
  return (
    <div className="relative">
      ...
      {error && (
        <p className="absolute top-full right-0 mt-2 w-56 text-[11px] text-red-400 font-geist text-right">
          {error}
        </p>
      )}
    </div>
...
export function HeroConnectButton() {
  const { step, connectAndSign } = useWalletAuth();

  return (
    <button
      onClick={connectAndSign}
      disabled={step === "connecting" || step === "signing" || step === "ready"}
